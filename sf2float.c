...
int main(int argc, char* argv[]) {
    ...
    printf("copying....\n");
    /* TODO: Multiframe
    int nframes = psf_sndSize(ifd);
    frames = (float*) malloc(prop.chans * sizeof(float) * nframes);
    framesread = psf_sndReadFloatFrames(ifd, frames, nFrames)
    */
    ...
    while(framesread == 1) {
        ...
        if(psf_sndWriteFloatFrames(ofd, frame, 1) != 1) {
            printf("Error waiting to outfile\n");
            error++;
            break;
        }
        /* <---- do any processing here! ------> */
        printf("framesread: %f - totalread: %f\n", framesread, totalread);
        framesread = psf_sndReadFloatFrames(ifd, frame, 1);
